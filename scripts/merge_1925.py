...
data_csv = "data.csv"
cleaned_csv = "volume_csvs/1925_cleaned.csv"

# 1. Read existing data.csv and filter out any existing 1925 records
header = None
records_to_keep = []
existing_count = 0
...
with open(data_csv, "r", encoding="utf-8") as f_data:
    ...
    header = next(reader)
    for row in reader:
        if row and row[0] == "1925":
            existing_count += 1
            continue
        records_to_keep.append(row)

print(f"Found and removed {existing_count} old 1925 records from {data_csv}.")

# 2. Read new clean 1925 records
new_records = []
...
print(f"Successfully merged {len(new_records)} new clean 1925 records into {data_csv}.")
